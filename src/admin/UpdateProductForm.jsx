...
const UpdateProductForm = ({ ele }) => {
  ...
  const onSubmit = (data) => {
    ...
    axios
      .put(
        `https://ecommerce-api-hlp7.onrender.com/api/product/${ele._id}`,
        myData,
        {
          headers: {
      ...
      .then((res) => {
        ...
        console.log(e)
        fireToast("faild to update product", "error")
      }).finally(()=>{
        setload(false)
      })
  };
      ...
      <div className="container">
        <div className="max-w-[500px] mx-auto shadow-xl shadow-slate-400 bg-white rounded-md p-5 sm:px-10">
          <h1 className="text-2xl font-bold text-center">
            {ele ? "Update Book" : "Add Book"}
          </h1>
          <form action="" onSubmit={handleSubmit(onSubmit)} className="mt-5">
            <div className="flex justify-between">
              <label
                htmlFor="imageCover"
                className="p-2 px-3 w-fit font-bold text-white bg-black rounded-md cursor-pointer"
              >
                Choose a File
              </label>
              <input
                ...
                id="imageCover"
                onChange={(e) =>
                  setData({ ...myData, image: e.target.files[0] })
                }
              />
              <div className="">
                <img
                  src={
                    myData.image
                      ? URL.createObjectURL(myData.image)
                      : ele
                      ? ele?.image
                      : DefaultImg
                  }
            ...
            <div className="mt-3">
              ...
              <input
                ...
                onInput={(e) => setData({ ...myData, title: e.target.value })}
                id="title"
                {...register("title",)}
                className="w-full h-10 px-2 border-2 border-gray-400 rounded-lg outline-none"
              />
              {errors.title?.message && (
                <p className="h-5 text-red-500 animate-bounce">
                  {errors.title?.message}
                </p>
              )}
            </div>
            <div className="mt-3">
              ...
              <input
                ...
                onInput={(e) => setData({ ...myData, price: +e.target.value })}
                id="price"
                {...register("price")}
                className="w-full h-10 px-2 border-2 border-gray-400 rounded-lg outline-none"
              />
            ...
            <div className="mt-3">
              ...
              <input
                ...
                onInput={(e) =>
                  setData({ ...myData, quantity: +e.target.value })
                }
                id="quantity"
                {...register("quantity")}
                className="w-full h-10 px-2 border-2 border-gray-400 rounded-lg outline-none"
            ...
            <div className="mt-3">
              ...
              <select
                ...
                onInput={(e) =>
                  setData({ ...myData, category: e.target.value })
                }
                id="category"
                {...register("category")}
                className="w-full h-10 px-2 border-2 border-gray-400 rounded-lg outline-none"
            ...
            <div className="mt-3">
              ...
              <textarea
                ...
                className="w-full h-16 py-1 px-2 border-2 border-gray-400 rounded-lg outline-none resize-none"
              />
            ...
            <button
              type="submit"
              className="p-2 px-3 font-bold text-white bg-blue-500 rounded-md mt-3"
            >
              {load ? (
                <div className="flex justify-center">
